...
public:
    ...
    {
        if (this->_primeiro->_dado == umDado)
        {
            Elemento<T> *prox = this->_primeiro->_proximo;
            delete this->_primeiro;
            this->_primeiro = prox;
            this->_tamanho--;
            return umDado;
        ...
        Elemento<T> *element = this->_primeiro;
        Elemento<T> *elAnterior = nullptr;
        for (int i = 1; i <= this->_tamanho; i++)
        {
            if (element->_dado == umDado)
            {
                elAnterior->_proximo = element->_proximo;
                delete element;
                this->_tamanho--;
                return umDado;
            }
